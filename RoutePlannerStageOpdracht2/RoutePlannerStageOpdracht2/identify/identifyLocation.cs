...
namespace RoutePlanner.identify
{
    class identifyLocation
    {
        public string[,] identifyPosPerLocation(string[,] IOlist)
        {
            ...
            //create new working table for datastorage
            ...
            SetData.SetData(updateStr, Database, iP, gebruikersnaam, wachtwoord);
            // fill the database with the io list
            ...
            //throw away all useless values
            updateStr.CommandText = "DropGarbage";
            SetData.SetData(updateStr, Database, iP, gebruikersnaam, wachtwoord);
            //get all locations out of the database
            ...
            //add all the information in the array
            for (int i = 0; i < ConnectedLocation.GetLength(0); i++)
            {
                ...
                for (int j = 0; j < AfduwerBinnenbaan2.Length; j++)
                {
            ...
            //print eerst alle waarden uit voordat deze gereturned worden
